...
namespace UniModules.UniCore.EditorTools.Editor.Controls.ReorderableList {
	using System;
    ...
    public class ActionListAdaptor<T> : GenericListAdaptor<T>
    {
        private readonly Func<int,float> _heightFunction;

        public ActionListAdaptor(IList<T> list, ReorderableListControl.ItemDrawer<T> itemDrawer,Func<int,float> heightFunction,float minHeight) : 
            base(list, itemDrawer, minHeight)
        {
        ...
        {
            var height = _heightFunction==null ? 0 : _heightFunction(index);
            return height < FixedItemHeight ? FixedItemHeight : height;
        }
    }
}
